      ...
      for (let elt of jsonObject) {
        ...
        if (elt.Category_name != "") {
          ...
          for (let iC = 1; iC <= iNodeCat; iC++) {
            if (elt["AttributeOfCat" + iC] != "") {
              let strAttr = elt["AttributeOfCat" + iC].split(";");
              if (tabCats.includes(strAttr[0]) == true) {
                let categoryAttribute = await AttributeService.getCategoryByName(category, strAttr[0]);
                AttributeService.addAttributeByCategory(category, categoryAttribute, strAttr[1], strAttr[4], strAttr[2], strAttr[3]);
              }
              else {
                let categoryAttribute = await AttributeService.addCategoryAttribute(category, strAttr[0]);
        ...
        // groups
          ...
          for (let iG = 1; iG <= iNodeGrp; iG++) {
            if (elt["AttributeOfGroup" + iG] != "") {
              ...
              if (tabCats.includes(strAttr[0]) == true) {
                let categoryAttribute = await AttributeService.getCategoryByName(groupNode, strAttr[0]);
