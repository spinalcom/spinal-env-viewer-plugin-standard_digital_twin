...
class StandardNomenclatureContext {
  static initialize() {
    ...
    StandardNomenclatureContext.initialized = new Promise((resolve, reject) => {
      StandardNomenclatureContext.context = SpinalGraphService.getContext(
        StandardNomenclatureContext.contextName
      );
      if (typeof StandardNomenclatureContext.context === 'undefined') {
        SpinalGraphService.addContext(
          StandardNomenclatureContext.contextName,
          StandardNomenclatureContext.type,
          new Model({
          ...
          for (let group of category.groups) {
            ...
            for (let config of group.conf) {
              ...
              SpinalGraphService.addChildInContext(
                groupId,
                nomenclatureConfigurationId,
                this.contextId,
                'groupHasAttributeConfiguration',
                SPINAL_RELATION_PTR_LST_TYPE
              ).then((result) => {
                let realNode = SpinalGraphService.getRealNode(
                  nomenclatureConfigurationId
                );
