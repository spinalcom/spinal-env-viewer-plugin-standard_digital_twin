...
class StandardEquipmentContext {
  static initialize() {
    ...
    StandardEquipmentContext.initialized = new Promise((resolve, reject) => {
      StandardEquipmentContext.context = SpinalGraphService.getContext(
        StandardEquipmentContext.contextName
      );
      if (typeof StandardEquipmentContext.context === 'undefined') {
        SpinalGraphService.addContext(
          StandardEquipmentContext.contextName,
          StandardEquipmentContext.type
        )
          .then((context) => {
  ...
  static async generateFromExcel(dataJson) {
    ...
    return this.initialize()
      .then(async (result) => {
        ...
        let iNodeCat = keyTab.filter((elt) => elt.includes('AttributeOfCat'))
          .length;
        let iNodeGrp = keyTab.filter((elt) => elt.includes('AttributeOfGroup'))
          .length;
        //parcours du tableau
        for (let elt of jsonObject) {
          ...
          if (elt.Category_name != '') {
            ...
            category = await SpinalGraphService.addChildInContext(
              this.contextId,
              catNodeId,
              this.contextId,
              CONTEXT_TO_CATEGORY_RELATION,
