...
class StandardControlEndpointContext {
  static initialize() {
    ...
    StandardControlEndpointContext.initialized = new Promise(
      (resolve, reject) => {
        StandardControlEndpointContext.context = SpinalGraphService.getContext(
          StandardControlEndpointContext.contextName
        );
        if (typeof StandardControlEndpointContext.context === 'undefined') {
          SpinalGraphService.addContext(
            StandardControlEndpointContext.contextName,
            StandardControlEndpointContext.type
          )
            .then((context) => {
...
StandardControlEndpointContext.contextName =
  'Contexte de Control Endpoint Standardisé';
