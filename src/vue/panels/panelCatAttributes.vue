  ...
  <v-app>
    <v-layout row justify-center>
      <v-dialog v-model="dialog" max-width="290">
        <v-card :dark="true">
          <v-card-title class="headline">Category attributes</v-card-title>
          <v-list>
...
export default {
  ...
  methods: {
    initialize: function(categoryAtrributes) {
      this.catItem = categoryAtrributes;
      this.itemsAttr = this.getCatAttributes();
    },
    onCancel: function() {
      this.dialog = false;
    },
    ...
    getCatTypes() {
      let catTypes = [];
      let firstEl = "";
      let attr = this.catItem.attributes;
      if (!attr) return [];
      attr.forEach(element => {
        firstEl = element.split(";")[0];
        if (!catTypes.includes(firstEl) && firstEl !== "") {
          catTypes.push(firstEl);
        }
      });
      return catTypes;
    },
    /* category_label ; attribute_label ; attribute_type ; attribute_unit ; attribute_value */
    getCatAttributes() {
      let catAttr = [];
      let attr = this.catItem.attributes;
      for (let element of attr) {
        if (element != undefined && element != "") {
          ...
          const found = catAttr.find(el => el.name === arrayTest[0]);
          if (found) {
